...
// 18 66 78 36 12 D2 99
// 52.64C
// 18 66 87 96 13 23 A5
// 819144.81C
...
double calculate_humidity(uint8_t *data) {
  uint32_t humidity = 0;
  print_hex_value((uint32_t)data[1]);
  uart_printstr(" ");
  print_hex_value(((uint32_t)data[2]));
  uart_printstr(" ");
  print_hex_value(((uint32_t)data[3]) >> 4);
  uart_printstr(" ");
  uart_printstr("\r\n");

  humidity += (uint32_t)data[1] << 16;
  ft_putnbr(humidity);
  uart_printstr(" ");
  humidity += (uint32_t)(data[2]) << 8;
  ft_putnbr(humidity);
  uart_printstr(" ");
  humidity += (uint32_t)(data[3]) >> 4;
  ft_putnbr(humidity);
  uart_printstr(" ");
  return (((double)humidity / 1048576.0f) * 100.0f);
}
